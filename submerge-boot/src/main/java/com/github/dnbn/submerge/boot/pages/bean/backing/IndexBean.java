...
import com.github.dnbn.submerge.boot.constant.SupportedLocales;
...
public class IndexBean extends AbstractManagedBean implements Serializable {
	...
	public StreamedContent getGeneratedFile() {
		...
		if (subOne != null && subTwo != null) {
			...
			sc = DefaultStreamedContent.builder().contentEncoding("UTF-8").contentType("text/plain")
					.stream(() -> sub.toInputStream()).name(getFileName() + ".ass").build();
	 ...
	 * 
	 * @param renderError: if true, an error message is rendered if the subtitle is
	 *                     null
	 */
